...
> **本アプリケーションは、信頼できる閉域ネットワーク内での使用を前提としています。**

すべての通信経路が暗号化されない平文 HTTP で動作します:

| 通信経路 | 流れる認証情報 |
|---------|--------------|
| ブラウザ → Zabbix サーバ | ARI ユーザ名・パスワード（ウィジェット設定の HTTP 送受信） |
| ブラウザ → Asterisk サーバ | WebSocket URL に埋め込まれた ARI 認証情報（`?api_key=user:pass`） |
| Zabbix サーバ → Asterisk サーバ | PHP プロキシの curl による HTTP Basic 認証 |

認証情報と音声データがすべての経路で平文伝送されるため、**ブラウザ（オペレータの PC）・Zabbix サーバ・Asterisk サーバの3者は、同一の信頼できるネットワークゾーン**（閉域 LAN やプライベート VLAN など）に置く必要があります。これらのサービスをインターネットに公開してはいけません。
